...
const dibujarPelis = (peliculas) => {
	...
	Array.isArray(peliculas) && peliculas.length
		? peliculas.map((p) => {
				plantilla += `<li><h4 id='${p.episode_id}'>${p.title}</h4><p>Id: ${p.episode_id}</p></li>`;
		  })
...
const dibujarDetalle = (peliculas, id) => {
	...
	return `<div id='${p.episode_id}'><h4>${p.title}</h4><p>Sinopsis: ${p.opening_crawl}</p><p>Director: ${p.director}</p><p>Productor: ${p.producer}</p><p>Fecha de lanzamiento: ${p.release_date}<p/></div>`;
};
